...
@section('styles')
    ...
    <link href="{{ asset('template/assets/css/apps/mailing-chat.css') }}" rel="stylesheet" type="text/css" />
    ...
    <link href="{{ asset('template/assets/css/detalle/detalle.css') }}" rel="stylesheet" type="text/css" />
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-5">
            ...
            @include('incidencias.partials.detalle2')

            <a href="{{ route('incidencias.index') }}" class="btn btn-sm btn-primary"
                title="Lista de Incidencias">Regresar</a>
            <a href="{{ route('incidencias.edit', $incidencia) }}" class="btn btn-sm btn-secondary"
                title="Editar incidencia">Editar</a>

        </div>

        <div class="col-md-7">
            <div>
                {{-- <h5 class="text-bold mt-4">Chat</h5> --}}
                @livewire('chat-list', ['incidencia' => $incidencia])
                @livewire('chat-form', ['incidencia' => $incidencia])

                {{-- @livewire('chat-list' /* , ['user' => $user], key($user->id) */) --}}

                {{-- @livewire('chat', ['incidencia' => $incidencia]) --}}
...
@section('scripts')
    ...
    <script src="{{ asset('template/assets/js/apps/mailbox-chat.js') }}"></script>
@endsection
