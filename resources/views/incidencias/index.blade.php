...
@section('content')
    ...
    <div class="col">
        <div class="infobox-3" style="background-color: rgba(226,160,63,0.57)">
            <div class="info-icon">
                <img style="width: 50px; height: 50px; color: white" src="{{asset('template/assets/img/favicon.ico')}}"
                    alt="Grapefruit slice atop a pile of other slices">
            </div>
            <h5 class="info-heading">Incidencias</h5>
            <div class="counter-container">
            ...
            <tbody>
                @foreach ($incidencias as $incidencia)
                    <tr class="table-primary">
                        ...
                        <td class="text-center"><span class="badge outline-badge-dark{{-- primary --}}"
                                style="background-color: {{ $incidencia->statu->color2 }}">
                                {{ $incidencia->statu->nombre }} </span>
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center align-items-center">
                                <a class="bs-tooltip" href="{{ route('incidencias.show', $incidencia) }}"
                                    title="Mostrar"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                ...
                                <a class="bs-tooltip" href="{{ route('incidencias.edit', $incidencia->id) }}"
                                    title="Editar"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                ...
                                <form class="delete-form" action="{{ route('incidencias.destroy', $incidencia) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                    {{-- <button type="submit" class="bs-tooltip" title="Eliminar" --}}
                                    <button type="submit" onclick="confirmDelete({{ $incidencia->id }}) class=" bs-tooltip" title="Eliminar"
...
@section('scripts')
    ...
    {{-- verificar porque no muestra primero este sweetalert antes de eliminar --}}
    <script>
        function confirmDelete(incidenciaId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Si el usuario confirma, construye la ruta y envía la solicitud de eliminación
                    const deleteForm = document.querySelector('.delete-form');
                    deleteForm.action = "{{ route('incidencias.destroy', '') }}/" + incidenciaId;
                    deleteForm.submit();
                }
            });
        }
    ...
    {{-- @if (session('status') && in_array(session('status'), ['creada', 'actualizada', 'eliminada']))
        <script>
            Swal.fire({
                title: 'Incidencia ' + session('status') + '!',
                icon: 'success',
            })
        </script>
    @endif --}}
